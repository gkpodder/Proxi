...
def gmail_search(query: str, max_results: int = 10) -> List[Dict[str, Any]]:
    ...
    logger.info("Calling Gmail API: users.messages.list")
    resp = service.users().messages().list(userId="me", q=query, maxResults=max_results).execute()
    messages = resp.get("messages", [])
    results = []
    for msg in messages:
        logger.info("Calling Gmail API: users.messages.get id=%s", msg.get("id"))
...
def gmail_get_message(message_id: str) -> Dict[str, Any]:
    ...
    logger.info("gmail_get_message id=%s", message_id)
    service = get_gmail_service()
    logger.info("Calling Gmail API: users.messages.get")
...
def gmail_summarize(query: str, max_results: int = 5) -> Dict[str, Any]:
    ...
    logger.info("gmail_summarize query=%s max_results=%s", query, max_results)
    ...
    logger.info("Sending summary request to OpenAI model=%s", model)
