...
2026-04-06 & 1.3 & Final docs, implemented TA feedback \\
\bottomrule
\end{tabularx}
...
Companion documents for this design include the SRS, the Module Interface
Specification (MIS), and the V\&V Plan, all available in the public project
repository at \href{https://github.com/gkpodder/Proxi}{GitHub}.
...
This section provides an overview of the module design. The modules are
summarized below in a hierarchy decomposed by secrets, and their detailed
descriptions are provided later in Section \ref{SecMD}. The leaves in this
hierarchy are the modules that will actually be implemented.
...
\begin{description}
  ...
  \textbf{BH-Plan (Task Executor)} selects MCP tools and executes user
  intents.
  \item [\refstepcounter{mnum} \mthemnum \label{mBHSafety}:]
  \textbf{BH-Safety (Confirmation Gate)} validates risky actions and requests
  confirmation as needed.
  \item [\refstepcounter{mnum} \mthemnum \label{mBHSession}:]
  \textbf{BH-Session (Context Manager)} stores history, session context, and
  undo information.
  \item [\refstepcounter{mnum} \mthemnum \label{mBHFeedback}:]
  \textbf{BH-Feedback (Response Manager)} outputs responses using TTS or visual
  text.
  \item [\refstepcounter{mnum} \mthemnum \label{mBHUI}:]
  \textbf{BH-UI (Proxi Interface)} displays status, confirmation prompts, and
  results.

  % SD
  \item [\refstepcounter{mnum} \mthemnum \label{mSDTypes}:]
  \textbf{SD-Types (Core Structures)} defines shared type and record definitions
  for Command, Intent, Plan, and related structures.
  \item [\refstepcounter{mnum} \mthemnum \label{mSDRegistry}:]
  \textbf{SD-ToolRegistry (Action Map)} maps intents to MCP tools and actions.
...
\subsubsection{BH-Input: Voice \& Text Manager (\mref{mBHInput})}
\begin{description}
...
\subsubsection{BH-Feedback: Response Manager (\mref{mBHFeedback})}
\begin{description}
...
\subsection{Shared Type Definitions and Software Decision Modules (\mref{mSDTypes})-(\mref{mSDLog})}

These modules define shared domain representations and system-wide services such
as tool mapping, persistence, AI service integration, and logging.
...
and tool metadata.
\item[Services:] Defines the shared type and record definitions used system-wide.
\item[Implemented By:] \progname{}
\item[Type Of Module:] Library
\end{description}
...
anticipated changes. Table~\ref{TblRT} maps functional requirements to modules,
Tables~\ref{TblNFRRT} and \ref{TblNFRRTCont} summarize the nonfunctional
coverage, and Table~\ref{TblACT} links anticipated changes to the modules that
hide them. These references should be read alongside the detailed module
descriptions in Section~\ref{SecMD}.
...
\section{Uses Hierarchy Between Modules} \label{SecUse}
...
implementation of B. Figure~\ref{FigUH} illustrates the uses relation between
the modules. The different node shapes distinguish the hardware-hiding,
behaviour-hiding, and shared type / software-decision modules. The graph is a
directed acyclic graph (DAG), and each level of the hierarchy offers a testable
and usable subset of the system.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth,trim=50 0 0 0,clip]{UsesHierarchy.pdf}
\caption{Uses hierarchy among modules. Distinct node shapes indicate different module categories.}
\label{FigUH}
\end{figure}

\section{User Interfaces}

This section goes over the user interfaces. The UI is designed to support a
voice-first interaction while also allowing keyboard and mouse input as
secondary options.

\subsection{Main Desktop Panel}

The primary interface is a desktop window that can remain visible while the
user works in other applications. It can also run in the background and listen
to the user. Figure~\ref{FigMainUI} shows a mockup of this interface. The panel
provides:
...
based on the Module Guide. The identifiers `M1`--`M14` refer to the leaf-module
ordering established in Section~\ref{SecMH} and are used consistently with the
MIS hierarchy.
...
\begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash}p{0.16\textwidth}|>{\raggedright\arraybackslash}p{0.22\textwidth}|>{\raggedright\arraybackslash}p{0.14\textwidth}|>{\raggedright\arraybackslash}X|}
\hline
\textbf{Date} & \textbf{Module(s)} & \textbf{Lead(s)} & \textbf{Details} 
\\ \hline
Nov 15 - Nov 29  & M13 (SD-AIClient), M14 (SD-Log) & Ajay & \textbf{Setup:} Environment configuration and 
implementation of \texttt{log\_event()} for system-wide traceability 
(FUNC.R.7). \\ \hline
Nov 30 - Dec 9 & M10 (SD-Types) & Gourob & \textbf{Type-Definition:} Coding of shared 
type and record definitions (\texttt{Command}, \texttt{Intent}, \texttt{Plan}) to ensure type-safety 
across the pipeline. \\ \hline
Dec 10 - Dec 19 & M1 (HH-IO) & Amanbeer & \textbf{Signal-Capture:} Low-level implementation 
of \texttt{init\_audio()} and \texttt{start\_capture()} using hardware-hiding 
drivers. \\ \hline
Dec 20 - Dec 31 & M1 (HH-IO) & Amanbeer & \textbf{Feedback-Link:} Implementation 
...
\\ \hline
Jan 1 - Jan 9 & M3 (BH-Input) & Savinay & \textbf{Stream-to-Text:} Integration of Whisper STT; 
implementation of VAD (Voice Activity Detection) logic. \\ \hline
Jan 10 - Jan 19 & M3 (BH-Input), M4 (BH-NLU) & Savinay, Gourob & \textbf{Normalization:} Unifying voice and text 
inputs into a common \texttt{Command} object via \texttt{get\_input()}. \\ 
\hline
Jan 20 - Jan 31 & M4 (BH-NLU) & Gourob & \textbf{Semantic-Parser:} Development of rules to map natural 
language strings to structured \texttt{Intent} objects. \\ \hline
Feb 1 - Feb 9 & M11 (SD-ToolRegistry), M12 (SD-Store) & Ajay & \textbf{Action-Map:} Building the \texttt{SD-ToolRegistry} 
and initial \texttt{SD-Store} persistence hooks for mapped intents and saved state. \\ \hline
\end{tabularx}
...
Feb 10 - Feb 19 & M5 (BH-Plan) & Gourob & \textbf{Logic-Sequence:} Implementation of recursive planning 
to decompose intents into atomic \texttt{Action} sequences. \\ \hline
Feb 20 - Feb 28 & M2 (HH-Auto) & Amanbeer & \textbf{OS-Abstraction:} Development of keyboard and 
mouse automation wrappers (click, type, launch) for Windows/macOS. \\ \hline
Mar 1 - Mar 9 & M6 (BH-Safety) & Savinay & \textbf{Gate-Keeper:} Mandatory implementation of 
\texttt{validate\_plan()} to intercept high-risk/irreversible actions 
(FUNC.R.9). \\ \hline
Mar 10 - Mar 19 & M7 (BH-Session) & Ajay & \textbf{Context-Sync:} Implementing conversational memory to 
...
\hline
\textbf{Week} & \textbf{Module(s)} & \textbf{Lead(s)} & \textbf{Details} \\ \hline
Mar 20 - Mar 30 & M9 (BH-UI) & Savinay & \textbf{View-Render:} Finalizing the Desktop Panel; rendering 
transcript overlays and real-time status indicators. \\ \hline
Mar 31 - Apr 9 & M8 (BH-Feedback) & Amanbeer & \textbf{Voice-Synth:} Integration of TTS to convert system 
responses into spoken audio via the M1 interface. \\ \hline
Apr 10 - Apr 19 & All modules & Full Team & \textbf{V\&V:} Execution of the Confidence Protocol; 
