...
2026-04-06 & 1.5 & Final docs, implemented TA feedback \\
\bottomrule
\end{tabularx}
...
\noindent
InputError = \{\text{InputInitFailed}, \text{MicUnavailable},
\text{AlreadyCapturing}, \text{NotCapturing}, \text{NoInputAvailable}\}
...
Let the abstract state of BH-Input be
\[
  \sigma = (\text{inputState}, \text{currentMode}, \text{lastText},
  \text{partialText}, \text{lastError})
\]
For every reachable state $\sigma$,
\[
  \text{inputState} \in \text{InputState}
\]
\[
  (\text{inputState} = \text{Idle}) \Rightarrow \text{partialText} = ""
\]
\[
  (\text{lastError} = null) \lor (\text{lastError} \in \text{InputError})
\]
...
\begin{itemize}
  \item pre: $\text{inputState} = \text{Idle}$.
  \item transition:
  \[
    \text{inputState}, \text{currentMode}, \text{partialText}, \text{lastError}
    := \text{Listening}, \text{mode}, "", null
  \]
...
\textbf{stopCapture}():
\begin{itemize}
  \item pre: $\text{inputState} \neq \text{Idle}$.
  \item transition:
  \[
    \text{inputState}, \text{lastText}, \text{partialText}
    := \text{Idle}, \text{partialText}, ""
  \]
...
\begin{itemize}
  \item $out := s$, where
  \[
    s.\text{state} = \text{inputState} \land
...
Let
\[
  S = \text{InputState}, \quad
  E = \{\text{StartCmd}, \text{StopCmd}, \text{Chunk}, \text{Error},
  \text{Timeout}\}, \quad
  s_0 = \text{Idle}
\]
The BH-Input controller is modelled as the finite-state machine
\[
  M = (S, E, s_0, \delta)
\]
where $\delta = \text{nextState}$.

\noindent
$\text{nextState} : \text{InputState} \times E \rightarrow \text{InputState}$
...
The transition function is total and deterministic because all remaining pairs
map to $s$ through the otherwise clause. During execution, BH-Input updates
\[
  \text{inputState} := \text{nextState}(\text{inputState}, e)
\]
for each event $e$, and when transcription ends, $\text{partialText}$ moves
into $\text{lastText}$.
...
than full natural language understanding. Our pipeline is also fairly
sequential, so errors in speech capture or intent parsing can propagate
forward into planning and execution. The system also depends on
...
would strengthen context handling between modules, improve recovery
when one stage of the pipeline fails, and explore more robust language
models, better noise handling, and wider accessibility testing with real users.
We would also add more automation around logging and replaying user sessions for
regression testing, drawing from ideas in my QA work and SFWRENG 3S03 and
...
enviroment and heavlily is reliant on STT and LLM. The current design also uses
a relatively simple interaction loop, so more complex multi-step tasks or longer
conversations may require repeated clarification from the user. Hence, offline use cases
and strong privacy guarantees are not well supported. Given unlimited resources,
we would strengthen local processing, improve context handling across the system,
and add richer accessibility features to target a broader user base.
...
for our project. Our design also still depends on a chain of separate modules for
input, interpretation, planning, and safety, so failures in one stage can reduce
the reliability of the full interaction. Given unlimited time, we would aim to
improve the coordination between these stages and create stronger fallback behaviour
before attempting our own model for this specific use-case. Making API calls for every single
...
action planning tasks. At the design level, another limitation is that our current
architecture still keeps context memory, action planning, and safety confirmation
fairly lightweight, which can make longer or more ambiguous user requests harder
to handle consistently. These models are optimized to understand and execute
...
low latency and better user experience. With more resources, we would also build
stronger local context management and more robust recovery paths for failed plans.
