...
\midrule
April 5, 2026 & Savinay & Initial User Manual release with interface startup, TUI/GUI settings, MCP setup, and Discord integration guidance\\
...
This manual describes the recommended workflow for Proxi:
\begin{enumerate}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{images/gateway_run_command.png}
\caption{Gateway startup command in terminal.}
\label{fig:gateway-start}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{images/proxi_tui.png}
\caption{Proxi TUI interface after launch.}
\label{fig:tui-home}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{images/proxi_gui_homescreen.png}
\caption{Proxi GUI home screen.}
\label{fig:gui-home}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{images/tui_slash_commands.png}
\caption{TUI slash command interface and command discovery flow.}
\label{fig:tui-slash-commands}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{images/gui_settings_general.png}
\caption{GUI Settings: General section.}
\label{fig:gui-settings-general}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{images/gui_settings_profile.png}
\caption{GUI Settings: Profile section.}
\label{fig:gui-settings-profile}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{images/gui_settings_api_keys.png}
\caption{GUI Settings: API Keys section.}
\label{fig:gui-settings-api-keys}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{images/gui_settings_integrations.png}
\caption{GUI Settings: Integrations section.}
\label{fig:gui-settings-integrations}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{images/gui_settings_cron_jobs.png}
\caption{GUI Settings: Cron Jobs section.}
\label{fig:gui-settings-cron-jobs}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{images/gui_settings_webhooks.png}
\caption{GUI Settings: Webhooks section.}
\label{fig:gui-settings-webhooks}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{images/gui_settings_voice.png}
\caption{GUI Settings: Voice section.}
\label{fig:gui-settings-voice}
\end{figure}
...
This section covers common issues and quick recovery steps.

\subsection{Gateway or Interface Won't Connect}

\textbf{Symptoms}
\begin{itemize}
\item GUI shows disconnected status
\item TUI does not respond after startup
\item \texttt{uv run proxi frontend} exits early
\end{itemize}

\textbf{Fix}
\begin{enumerate}
\item Start gateway first: \texttt{uv run proxi gateway start}
\item Start your interface after gateway is running
\item Ensure only one gateway process is active
\item If needed, stop and restart gateway, then relaunch TUI/GUI
\end{enumerate}

\subsection{Voice Input Is Misheard or Not Triggering}

\textbf{Symptoms}
\begin{itemize}
\item Wake word does not activate listening
\item Spoken text is inaccurate
\item Mic button starts but no transcript appears
\end{itemize}

\textbf{Fix}
\begin{enumerate}
\item In GUI \textbf{Settings \textgreater\ Voice}, confirm wake word is enabled
\item Allow microphone permission in your browser
\item Reduce background noise and speak more clearly
\item Use the Mic button manually if wake-word activation is unreliable
\item Increase auto-stop sensitivity if recording ends too quickly
\end{enumerate}

\subsection{Integration Commands Fail (Gmail, Calendar, Spotify, etc.)}

\textbf{Symptoms}
\begin{itemize}
\item Commands return auth errors
\item Integration tools are unavailable
\item Actions fail after keys were recently changed
\end{itemize}

\textbf{Fix}
\begin{enumerate}
\item Verify required API keys are present in \textbf{Settings \textgreater\ API Keys}
\item Confirm the integration is enabled in \textbf{Settings \textgreater\ Integrations}
\item Re-run first-time authorization flow when prompted in browser
\item Send one simple validation prompt for that integration
\item Restart gateway if tool availability appears stale
\end{enumerate}

\subsection{Cron or Webhook Jobs Do Not Run}

\textbf{Symptoms}
\begin{itemize}
\item Scheduled prompts never trigger
\item Webhook calls are accepted but do not produce actions
\end{itemize}

\textbf{Fix}
\begin{enumerate}
\item Confirm the source/job is not paused
\item Validate cron format and target agent/session values
\item Re-check webhook secret configuration if signatures are enabled
\item Inspect gateway logs for source-specific errors
\end{enumerate}

\subsection{Setup or Dependency Problems}

\textbf{Symptoms}
\begin{itemize}
\item Missing dependencies after pulling new changes
\item Commands fail due to missing runtime packages
\end{itemize}

\textbf{Fix}
\begin{enumerate}
\item From repository root run: \texttt{uv sync}
\item Re-run setup: \texttt{uv run proxi setup}
\item Retry gateway startup and then launch your selected interface
\end{enumerate}
