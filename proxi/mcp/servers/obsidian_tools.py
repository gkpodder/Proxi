...
class ObsidianTools:
    ...
    def _discover_vaults(self) -> dict[str, Path]:
        ...
        env_single = os.getenv("OBSIDIAN_VAULT_PATH", "").strip()
        if env_single:
            path = Path(env_single).expanduser().resolve()
            if path.is_dir():
                vaults[path.name] = path

        env_multi = os.getenv("OBSIDIAN_VAULT_PATHS", "").strip()
        if env_multi:
            for raw_path in env_multi.split(os.pathsep):
    ...
    def _resolve_vault(
        ...
        if not self._vaults:
            raise ValueError(
                "No Obsidian vaults found. Set OBSIDIAN_VAULT_PATH/OBSIDIAN_VAULT_PATHS "
                "or open Obsidian to initialize obsidian.json."
            )
